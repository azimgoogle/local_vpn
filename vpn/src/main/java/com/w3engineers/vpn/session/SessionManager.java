...
import com.w3engineers.vpn.socket.DataConst;
import com.w3engineers.vpn.socket.SocketNIODataService;
import com.w3engineers.vpn.socket.SocketProtector;
...
//	}
	...
	public Session createNewUDPSession(int ip, int port, int srcIp, int srcPort){
		if(ip < 0 || port < 0 || srcIp < 0 || srcPort < 0)
			return null;
	...
	public Session createNewSession(int ip, int port, int srcIp, int srcPort){
		...
		try {
			...
			//Enables Nagle's Algorithm which increase network latency and decrease performance
